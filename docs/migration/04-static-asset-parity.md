...
- [x] Loading the branch-preview site and checking the browser tab icon, and a mobile "add to home screen" preview if convenient, shows the correct icon/theme color. **Follow-up commit `2fc4d4d` fixed the previously-found defect; re-verified independently.** `astro/public/favicon.ico` is now sha256-byte-identical to the real `static/favicon.ico` (`d5857a9c…`, 15086 bytes; `file` reports "MS Windows icon resource - 3 icons" for both — the 655-byte PNG-in-.ico placeholder is gone). `astro/public/favicon.svg` is now real potrace-traced site branding — byte-for-byte identical to `safari-pinned-tab.svg` (the `#b83854` logomark), confirmed with `diff` — not the default Astro "A" logomark. `astro/src/layouts/Layout.astro`'s `<link rel="icon" type="image/svg+xml" href="/favicon.svg">` and `<link rel="icon" href="/favicon.ico">` now resolve to these real assets, confirmed verbatim in a fresh `astro/dist/index.html` `<head>` after `npm run build`, and `dist/favicon.ico`/`dist/favicon.svg` are sha256-identical to their `public/` sources. Live branch-preview + mobile "add to home screen" visual confirmation still not done — no Netlify/device access in this environment — same outstanding manual step as T1–T3; local build and byte-level verification are solid enough to consider this item passed.
